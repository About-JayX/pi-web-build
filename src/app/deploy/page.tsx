...
import { FaRocket, FaLayerGroup, FaCheck, FaArrowRight, FaUpload, FaGlobe, FaTwitter, FaTelegram, FaDiscord, FaChevronDown, FaChevronUp, FaEdit, FaCog } from 'react-icons/fa'
...
const TokenParametersSection = ({ 
  ...
  const { isOpen: isTokenParamsOpen, onToggle: onTokenParamsToggle } = useDisclosure()
  ...
  return (
    <Box>
      ...
      <Flex 
        ...
        borderRadius="md"
      >
        <Heading size="sm" color={labelColor} fontWeight="semibold">{t('tokenParameters')}</Heading>
        <Box display="flex">
          {/* PC版显示编辑按钮 */}
          {!isMobile ? (
            <IconButton
      ...
      {/* 当前参数显示 */}
      <SimpleGrid columns={2} spacing={4} bg="gray.50" p={4} borderRadius="md" mt={2}>
        <Box>
          <Text fontSize="sm" color="gray.600" mb={1}>{t('totalSupply')}</Text>
          <Text fontWeight="bold" color={labelColor}>{totalSupplyTabIndex === 0 ? "314,000,000" : "1,000,000,000"}</Text>
        </Box>
        <Box>
          <Text fontSize="sm" color="gray.600" mb={1}>{t('mintingAmount')}</Text>
          <Text fontWeight="bold" color={labelColor}>{targetAmountOptions[totalSupply][targetAmountTabIndex]}</Text>
        </Box>
        <Box>
          <Text fontSize="sm" color="gray.600" mb={1}>{t('exchangeRate')}</Text>
          <Text fontWeight="bold" color={labelColor}>1 : {getFormattedPrice}</Text>
        </Box>
        <Box>
          <Text fontSize="sm" color="gray.600" mb={1}>{t('tokenPrice')}</Text>
          <Text fontWeight="bold" color={labelColor}>{getFormattedInversePrice} {currencyUnit}</Text>
        </Box>
      </SimpleGrid>
      ...
      {isMobile && (
        ...
        >
          <VStack spacing={5} align="stretch">
            <FormControl>
              <FormLabel color={labelColor} fontSize="sm" fontWeight="semibold">{t('totalSupply')}</FormLabel>
              <Tabs 
            ...
            <FormControl>
              <FormLabel color={labelColor} fontSize="sm" fontWeight="semibold">{t('mintingAmount')}</FormLabel>
              ...
              {totalSupplyTabIndex === 0 ? (
                <Tabs 
                  ...
                  size="sm"
                >
                  <TabList bg="gray.50" p={1} borderRadius="md" justifyContent="start">
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} mx={1}>314 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} mx={1}>157 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} mx={1}>78.5 {currencyUnit}</Tab>
                  </TabList>
                </Tabs>
              ) : (
                <Tabs 
                ...
                >
                  <TabList bg="gray.50" p={1} borderRadius="md" flexWrap="wrap">
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} m={1}>100 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} m={1}>200 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} m={1}>250 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} m={1}>400 {currencyUnit}</Tab>
                    <Tab _selected={{ color: 'white', bg: 'brand.primary' }} m={1}>500 {currencyUnit}</Tab>
                  </TabList>
                </Tabs>
              )}
            </FormControl>
  ...
  return (
    <Box>
      <Flex 
        ...
        borderRadius="md"
      >
        <Heading size="sm" color={labelColor} fontWeight="semibold">{t('socialLinks')}</Heading>
        <Icon 
      ...
      <Box display={isSocialOpen ? "block" : "none"} mt={4} pl={{ base: 0, md: 2 }}>
        <FormControl mb={4}>
          <FormLabel color="gray.600" fontSize="sm">{t('website')}</FormLabel>
          <Input 
            placeholder="" 
            bg={inputBg} 
        ...
        <FormControl mb={4}>
          <FormLabel color="gray.600" fontSize="sm">{t('twitter')}</FormLabel>
          <Input 
            placeholder="" 
            bg={inputBg} 
            borderColor={borderColor}
        ...
        <FormControl>
          <FormLabel color="gray.600" fontSize="sm">{t('telegram')}</FormLabel>
          <Input 
            placeholder="" 
            bg={inputBg} 
            borderColor={borderColor}
...
export default function DeployPage() {
  ...
  const targetAmountOptions = useMemo(() => {
    ...
    return {
      ...
      '1000000000': [`100 ${unit}`, `200 ${unit}`, `250 ${unit}`, `400 ${unit}`, `500 ${unit}`]
    };
  }, [currencyUnit]);
  
  const totalSupply: string = totalSupplyValues[totalSupplyTabIndex];
  const targetAmount: string = targetAmountOptions[totalSupply][targetAmountTabIndex];
  ...
  const formattedPrice = useMemo(() => {
    if (tokenPrice > 1000000) {
      ...
      return parseFloat(tokenPrice.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    } else {
  ...
  const getTokenPrice = () => {
    ...
    const targetAmountStr: string = targetAmountOptions[totalSupply][targetAmountTabIndex];
    ...
    if (price > 1000000) {
      ...
      formattedPrice = parseFloat(price.toFixed(2)).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    } else {
    ...
    if (inversePrice >= 1) {
      ...
      formattedInversePrice = formatNumberNoExponent(parseFloat(inversePrice.toFixed(8)));
    }
    
    return { formattedPrice, formattedInversePrice };
  }
  
  const { formattedPrice: getFormattedPrice, formattedInversePrice: getFormattedInversePrice } = getTokenPrice();
  ...
  return (
    <Box bg="brand.background" minH="100vh" color={textColor}>
      ...
      <Container maxW="container.md" px={{ base: 3, md: 6 }} pb={{ base: 12, md: 10 }}>
        <Card 
          bg={cardBg}
          boxShadow="md"
          borderRadius="lg"
            ...
            <Box width="100%">
              <VStack spacing={{ base: 4, md: 6 }} align="stretch">
                <SimpleGrid columns={{ base: 1, sm: 2 }} spacing={4}>
                  ...
                  <FormControl isRequired>
                    <FormLabel color={labelColor} fontWeight="semibold">{t('tokenName')}</FormLabel>
                    <Input 
                      placeholder="" 
                      bg={inputBg} 
                      borderColor={borderColor}
                ...
                <FormControl isRequired mb={4}>
                  <FormLabel color={labelColor} fontWeight="semibold">{t('tokenIcon')}</FormLabel>
                  <Box 
                    borderWidth="1px" 
                    borderStyle="dashed" 
                    borderColor={borderColor}
                    ...
                    mx="0"
                  >
                    <Icon as={FaUpload} fontSize="2xl" mb={3} color="brand.primary" />
                    <Text fontSize="sm" color="gray.500">
                      {t('uploadIcon')}<br/>
                      <span dangerouslySetInnerHTML={{ __html: t('iconRequirements') }} />
                    </Text>
                  </Box>
                </FormControl>
