...
const WalletConnectModal = ({
  ...
  //http://localhost:3000/points?code=PQU641FP
  ...
  const connectWallet = async (walletType: WalletType) => {
    ...
    try {
      ...
      if (!wallet) {
        // 获取下载链接
        const downloadUrl = WALLETS.find(
          (w) => w.id === walletType
        )?.downloadUrl
        ...
        toast({
          ...
          description: (
            <Box color="white">
              <Text mb={2} fontWeight="bold" fontSize="md" color="white">
                {t("pleaseInstallWallet", {
                  wallet: WALLETS.find((w) => w.id === walletType)?.name,
                })}
              </Text>
              <Button
                as="a"
