...
export function parseValueWithUnit(value: string | number): number {
  ...
  switch (unit) {
    case 'K': return numValue * 1e3;
    case 'M': return numValue * 1e6;
    case 'B': return numValue * 1e9;
    case 'T': return numValue * 1e12;
...
export function formatTokenAmount(
  ...
  // 转换为数字
  let numValue = typeof amount === 'string' ? parseValueWithUnit(amount) : amount;
...
export function formatTokenAmountByDecimals(
  ...
  // 1. 第一性原理：从原始值转换为实际值
  const numValue = typeof amount === 'number'
...
export function calculateMintingPrice(
  ...
  // SOL和Pi之间的精度差异处理
  const adjustedPrice = currencyUnit.toUpperCase() === 'SOL'
